...
/**
 * @minions-memory/cli — CLI for Minions memory
 *
...
import { customTypes } from '@minions-memory/sdk';
...
program.name('memory').description('Persistent cross-session memory, recalled facts, and agent knowledge base').version('0.1.0');

program.command('info').description('Show project info').action(() => {
    console.log(chalk.bold('Minions memory'));
    console.log(chalk.dim('Persistent cross-session memory, recalled facts, and agent knowledge base'));
    console.log('');
    console.log(`  SDK:    ${chalk.cyan('@minions-memory/sdk')}`);
    console.log(`  CLI:    ${chalk.cyan('@minions-memory/cli')}`);
    console.log(`  Python: ${chalk.cyan('minions_memory')}`);
